...
/* _TRAFICO_V1: acciones de negocio de Capacitaciones ETI */
(function(){
  var entro=false;
  setInterval(function(){try{var a=document.getElementById('appPage');if(a&&a.style.display!=='none'&&!entro){entro=true;RLTrafico.emit({accion:'Entró a Capacitaciones ETI',destino:'Firebase',user:(function(){var t=String((document.getElementById('userBadge')||{}).textContent||'').trim();return t?{u:t.toLowerCase().replace(/\s+/g,'.').slice(0,40),n:t}:null;})()});}}catch(e){}},2000);
  document.addEventListener('submit',function(ev){try{var f=ev.target;if(!f)return;var u=(function(){var t=String((document.getElementById('userBadge')||{}).textContent||'').trim();return t?{u:t.toLowerCase().replace(/\s+/g,'.').slice(0,40),n:t}:null;})();
    if(f.id==='etiForm'){var tema=(document.getElementById('fTema')||{}).value||'';RLTrafico.emit({accion:'Registró capacitación',destino:'Firebase',detalle:String(tema).slice(0,60),user:u});}
    else if(f.closest&&f.closest('#tab-mipanel')){RLTrafico.emit({accion:'Registró capacitación (Mi Panel)',destino:'Firebase',user:u});}
    else if(f.closest&&f.closest('#tab-programacion')){RLTrafico.emit({accion:'Programó capacitación',destino:'Firebase',user:u});}
  }catch(e){}},true);
})();
